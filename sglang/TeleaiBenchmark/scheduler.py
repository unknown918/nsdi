...
class Scheduler:
    ...
    def stop_profile(self) -> None:
        if self.profiler is None:
            raise RuntimeError("Profiler is not enabled.")
        self.profiler.stop()
        profile_file_name = os.getenv("PROFILE_FILE_NAME")
        PROFIL_DONE_FILE = os.getenv("PROFILE_DONE_FILE")
        print(f"profile_file_name: {profile_file_name}")
        import random
        random_num = random.randint(0, 10000)
        self.profiler.export_chrome_trace(
            self.torch_profiler_trace_dir + "/" + profile_file_name + "_" + str(random_num) + ".trace.json.gz"
        )
        # 设置环境变量flag
        with open(PROFIL_DONE_FILE, "w") as f:
            f.write("1")
        print("PROFILE_DONE set to 1")
