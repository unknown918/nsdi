...
class ServerArgs:
    ...
    # Expert parallelism
    ep_size: int = 1
    enable_ep_intra_node_reduce: bool = False
    ...
    attention_node_num: int = -1
    moe_node_num: int = -1
    num_expert_per_gpu: int = 30
    ...
    def __post_init__(self):
        ...
        print("[ServerArgs] chose attention_backend: ", self.attention_backend)
        # Expert parallelism
        ...
        # FIXME 8: 如果环境中gpu不是8需要改
        self.attention_node_num = math.ceil(self.tp_size / 8)
        self.moe_node_num = self.nnodes - self.attention_node_num
    ...
    def add_cli_args(parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            "--num-micro-batch",
            type=int,
            default=ServerArgs.num_micro_batch,
            help="The number of micro-batches to overlap within a single request.",
        )
        ...
        parser.add_argument(
            ...
            help="The attention node rank. If -1, no attention node is used."
        )
        parser.add_argument(
            "--expert-node",
            type=int,
            default=ServerArgs.expert_node,
            help="The expert node rank. If -1, no expert node is used."
        )
        ...
        parser.add_argument(
            "--enable-ep-intra-node-reduce",
            action="store_true",
            default=ServerArgs.enable_ep_intra_node_reduce,
            help="Enable EP intra-node all_reduce within MoE groups.",
        )
        parser.add_argument(
            "--moe-node-num",
            type=int,
            default=ServerArgs.moe_node_num,
            help="The number of MoE nodes.",
        )
        parser.add_argument(
            "--attention-node-num",
            type=int,
            default=ServerArgs.attention_node_num,
            help="The number of attention nodes.",
        )
        parser.add_argument(
            "--num-expert-per-gpu",
            type=int,
            default=ServerArgs.num_expert_per_gpu,
            help="Number of experts placed on one GPU"
        )
        # Model override args
        parser.add_argument(
