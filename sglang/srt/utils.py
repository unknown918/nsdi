...
def get_available_gpu_memory(device, gpu_id, distributed=False, empty_cache=True):
    ...
    if distributed:
        ...
        # att_worker_size = int(os.getenv("ATTN_WORKERS_COUNT"))
        # att_ranks = list(range(att_worker_size))
        att_ranks = list(range(2))
        my_rank = dist.get_rank()
        subgroup = dist.new_group(ranks=att_ranks)
        if my_rank in att_ranks:
            torch.distributed.all_reduce(tensor, op=torch.distributed.ReduceOp.MIN, group=subgroup)
        else:        
...
def monkey_patch_p2p_access_check():
    ...
    # Suppress the warnings from this delete function when using janus.bench_one_batch
...
def pytorch_profile(name, max_profile_steps, func, *args):
    """
    Args:
    ...
    if trace_dir is None or step_counter >= max_profile_steps:
        return func(*args)
    ...
    trace_file = os.path.join(trace_dir, f"{name}_{step_counter}.json")
...
def set_prometheus_multiproc_dir():
    ...
    # janus uses prometheus multiprocess mode
