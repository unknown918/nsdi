...
class GroupCoordinator:
    ...
    def __init__(
        ...
        if "ep" in group_name:
            # print(f"[GroupCoordinator] EP_GROUP local_rank={local_rank}  2")
            pass
        else:
...
# Expert-parallel group (for MoE ranks)
_EP: Optional[GroupCoordinator] = None
_EP_List: List[GroupCoordinator] = []

def get_ep_group(group_id: int) -> GroupCoordinator:    
    assert _EP_List[group_id] is not None, "expert model parallel group is not initialized"
    return _EP_List[group_id]

def init_expert_parallel_group(expert_model_parallel_size: int, moe_node_num: int = 1) -> None:
    """Initialize EP (expert-parallel) group only on EP ranks.    
    - Divides EP ranks into `moe_node_num` groups for intra-group communication.    
    """
    # global _EP
    global _EP_List
    # assert _EP is None, "expert model parallel group is already initialized"
    assert len(_EP_List) == 0, "expert model parallel group is already initialized"
 
    world_size: int = torch.distributed.get_world_size()
    backend = torch.distributed.get_backend(get_world_group().device_group)
    ep_ranks = list(range(world_size - expert_model_parallel_size, world_size))
    # print(f"[🦐] init_expert_parallel_group: ep_ranks={ep_ranks}")
    
    # 将EP ranks平分为moe_node_num个组
    ranks_per_node = expert_model_parallel_size // moe_node_num
    ep_group_ranks = []
    for i in range(moe_node_num):
        start_idx = i * ranks_per_node
        end_idx = start_idx + ranks_per_node
        if i == moe_node_num - 1:  # 最后一组包含所有剩余的ranks
            end_idx = expert_model_parallel_size
        group_ranks = ep_ranks[start_idx:end_idx]        
        ep_group_ranks.append(group_ranks)       
        # print(f"🌹[init_expert_parallel_group] group_ranks={group_ranks}, group_name=ep_{i}, my_rank={torch.distributed.get_rank()}")                        

        _EP_List.append(init_model_parallel_group(
            [group_ranks],
    ...
    # print(f"[🦐] {torch.distributed.get_rank()} fisish")
...
def init_attn_distributed_environment(
    world_size: int = -1,
    rank: int = -1,
    distributed_init_method: str = "env://",
    local_rank: int = -1,
...
def initialize_model_parallel(
    ...
    if world_size != tensor_model_parallel_size * pipeline_model_parallel_size + expert_model_parallel_size:
        raise RuntimeError(
    ...
    num_tensor_model_parallel_groups: int = (world_size - expert_model_parallel_size) // tensor_model_parallel_size
    global _TP
    ...
    print(f"group_ranks: {group_ranks}, get_world_group().local_rank: {get_world_group().local_rank}")
