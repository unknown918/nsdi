...
from sglang.CUHKSZ.disaggregate.runner.moe_runner import MoERunner
...
class MoeScheduler:
    def __init__(
        ...
        self.serer_args = server_args
        self.ep_rank = ep_rank
        
        self.ep_worker = MoERunner(
            server_args=server_args,
            gpu_id=gpu_id,
            ep_rank = ep_rank,
            port_args=port_args.nccl_port,
        )
...
class Scheduler:
    ...
    def __init__(
        ...
        print("[scheduler.py] apply SchedulePolicy")
    ...
    def event_loop_normal(self):
        ...
        print("[scheduler.py] event_loop_normal")
    ...
    def event_loop_overlap(self):
        print("[scheduler.py] event_loop_overlap")
...
def run_moe_scheduler_process(
    ...
    setproctitle.setproctitle("sglang::moe::scheduler")
    faulthandler.enable()    
    parent_process = psutil.Process().parent()    
    try:
        print(f"run_moe_scheduler_process: ep_rank={ep_rank}, gpu_id={gpu_id}")
        scheduler = MoeScheduler(server_args, port_args, gpu_id, ep_rank+server_args.tp_size)
        # pipe_writer.send(
        #     {
        #         "status": "ready",
...
def run_scheduler_process(
    ...
    print("[scheduler.py] run_scheduler_process")
    setproctitle.setproctitle("sglang::attn::scheduler")
    faulthandler.enable()
    suppress_other_loggers()
    parent_process = psutil.Process().parent()
