...
class DefaultModelLoader(BaseModelLoader):
    ...
    def _get_weights_iterator(
        self, source: "Source", model_config: Optional[ModelConfig] = None
        ...
        if self.load_config.load_format == LoadFormat.NPCACHE:
            ...
            logger.info(
                f"[Loader.py] (_get_weights_iterator) Call Function safetensors_weights_iterator"
            )
            weights_iterator = safetensors_weights_iterator(hf_weights_files=hf_weights_files, model_config=model_config)
        else:
    ...
    def _get_all_weights(
        ...
        yield from self._get_weights_iterator(primary_weights, model_config)

        secondary_weights = cast(
            Iterable[DefaultModelLoader.Source], getattr(model, "secondary_weights", ())
        )

        # print(getattr(model, "secondary_weights", ()))
    ...
    def load_model(
        ...
        *,
        ...
        with set_default_torch_dtype(model_config.dtype):
            ...
            logger.info(
                f"[Loader.py] (load_model) call get_all_weighs call model.load_weights"
            )
...
def get_model_loader(load_config: LoadConfig) -> BaseModelLoader:
    ...
    logger.info(
        f"[Loader.py] (get_model_loader) return DefaultModelLoader"
    )
