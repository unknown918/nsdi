...
def safetensors_weights_iterator(
    ...
    if model_config is None:        
        raise ValueError("model_config should be provided")        
    ...
    for st_file in tqdm(
        hf_weights_files,
        # desc="[Weight_utils.py] (safetensors_weight_iterator) Loading safetensors checkpoint shards [!! use yield, see get_tensor]",
        desc="[Weight_utils.py] Loading safetensors checkpoint shards",
        disable=not enable_tqdm,
        bar_format=_BAR_FORMAT,
    ):
        # FIXME zhexiang        
        NUM_SAFETENSOR_FILES = int(os.environ.get("NUM_SAFETENSOR_FILES", 1000))
        
        # in case of we havent download all the model safetensor files...
        filename = os.path.basename(st_file)            
        file_id = int(filename.split("-")[1].split("of")[0])
        if file_id >= NUM_SAFETENSOR_FILES:
            continue
        
        if not is_all_weights_sharded:            
            with safe_open(st_file, framework="pt") as f: # 打开权重文件                                
                for name in f.keys():  # noqa: SIM118                                                                                                                                                          
                    if name.startswith("model.layers"):   
                        layer_index = int(name.split(".")[2]) # (zx) 更通用的表达                                                                  
                        if layer_index >= model_config.hf_config.num_hidden_layers: continue  
                        
                        if os.environ.get("DISAGGREGATE_MODE") == "Attention":                            
                            if ".experts." in name: continue 
                        if ".experts." in name: continue 
                        # print(f"name: {name}")
                        if os.environ.get("DISAGGREGATE_MODE") == "Moe":                                                        
                            if ".experts." in name:                                   
                                param = f.get_tensor(name)
                                yield name, param     
                            else: continue                                                                                                   

                        # expert_used = model_config.hf_config.num_experts
                        expert_used = model_config.hf_config.n_routed_experts if "n_routed_experts" in model_config.hf_config else model_config.hf_config.num_experts
                        if ".experts" in name:                                
                            expert_index = int(name.split(".")[5])  # (zx) 更通用的表达                                 
                            if expert_index >= expert_used: continue                                 
                            param = f.get_tensor(name)
                        elif "mlp.gate.weight" in name:                             
                            param = f.get_tensor(name)[:expert_used] # (zx) 更通用的表达，gate维度与experts数目对应                        
                        elif "e_score_correction_bias" in name:
                            param = f.get_tensor(name)[:expert_used] #  
                        else:
                            param = f.get_tensor(name)
                    else:
                        param = f.get_tensor(name)                    
