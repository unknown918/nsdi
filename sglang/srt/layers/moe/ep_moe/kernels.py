...
def grouped_gemm_triton_kernel(
    ...
    # 大致是1个线程处理一个expert。如果某expert不在范围内或者没有要处理的token，则直接跳过计算。不会load，后面的b开始load对应expert的权重
    ...
    # 通过seg_indptr_cur_rank计算每个计算块的实际处理范围，如果范围为空（即当前专家没有要处理的token），就直接跳过计算。
    # 据线程块ID确定处理范围和对应专家Id
    ...
    # 根据expert_id动态地选择权重矩阵的对应部分，而expert_id是通过compute_m_range函数计算得到的。
    # expert_id * b_stride_0，它动态计算了权重矩阵中特定专家的起始位置。
    # b: 权重矩阵的基地址（起始指针）
    # expert_id * b_stride_0: 选择特定专家的权重子矩阵
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        # 从全局内存加载权重数据到本地，但仅加载当前计算实际需要的权重数据。
        b_tile = tl.load(
